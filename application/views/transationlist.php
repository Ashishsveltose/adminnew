...
include('sidebar.php');
        ...
        <div class="page-wrapper">
           ...
           <!-- <a href="https://ezheal.in/new_medical/transation" class="btn btn-primary float-right btn-rounded"><i class="fa fa-plus"></i> Add Transation</a>-->
            
                    <div class="col-lg-12">
                        <div class="card-box" style="margin-top:50px;">
                            <div class="card-block">
                                <h4 class="card-title">Transaction Tables</h4>
          ...
          <tr>
              ...
              <tbody>
                ...
                <?php $count = 1; 
                  if(!empty($transationlist)){
                    foreach ($transationlist as $getdata) { 
                  ?>
                    <tr>
                      <td><?php echo $count; ?></td>
                      <td><?php  echo (!empty($getdata['customer_id'])?$getdata['customer_id']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['purpose'])?$getdata['purpose']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".delete").click(function(e){
    var val = confirm("Are you sure, you want to delete Transaction ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'customer_transation',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate Transaction ?");
        if (val == true){
      window.location.reload();
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'customer_transation',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate Transaction ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'customer_transation',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
