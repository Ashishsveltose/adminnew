...
include('sidebar.php');
        ...
        <div class="page-wrapper">
         ...
         <!--   <a href="https://ezheal.in/admin/clinicdetails" class="btn btn-primary float-right btn-rounded"><i class="fa fa-plus"></i> Add Clinic</a>-->
            
                    <div class="col-lg-12">
                        <div class="card-box" style="margin-top:50px;">
                            <div class="card-block">
                                <h4 class="card-title">Withdrawal request Tables</h4>
              ...
              <tbody>
                ...
                <?php $count = 1; 
                  if(!empty($requestlist)){
                    foreach ($requestlist as $getdata) { 
                  ?>
                    <tr>
                      <td><?php echo $count; ?></td>
                      <td><?php  echo (!empty($getdata['usertype'])?$getdata['usertype']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['usrname'])?$getdata['usrname']:'none'); ?></td>
                      <td><?php  echo (!empty($getdata['amount'])?$getdata['amount']:'none'); ?></td>
                      
                      <td><?php  echo (!empty($getdata['payment_mode'])?$getdata['payment_mode']:'none'); ?></td>
                    
                     <td><?php  echo (!empty($getdata['upi'])?$getdata['upi']:'none'); ?></td>
                     <td><?php  echo (!empty($getdata['payement_date'])?$getdata['payement_date']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".delete").click(function(e){
    var val = confirm("Are you sure, you want to delete Withdrawal request ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'request',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate Withdrawal request ?");
        if (val == true){
      window.location.reload();
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'request',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to Withdrawal request ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'request',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
