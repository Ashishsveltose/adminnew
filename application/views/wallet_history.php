                    ...
                    <div class="col-lg-12">
                        <div class="card-box" style="margin-top:50px;">
                            <div class="card-block">
                                <h4 class="card-title">Wallet History Details Tables</h4>
              ...
              <tbody>
                ...
                <?php $count = 1; 
                  if(!empty($wallethistory)){
                    foreach ($wallethistory as $getdata) { 
                  ?>
                    <tr>
                      ...
                      <td><?php  echo (!empty($getdata['source'])?$getdata['source']:'none'); ?></td>
                    
                     <td><?php  echo (!empty($getdata['transaction_id'])?$getdata['transaction_id']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".delete").click(function(e){
    var val = confirm("Are you sure, you want to delete wallet history ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'wallet_history',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate clinic details ?");
        if (val == true){
      window.location.reload();
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'wallet_history',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to clinic details ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'wallet_history',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
