...
include('sidebar.php');
        ...
        <div class="page-wrapper">
            ...
            <a href="https://ezheal.in/admin/category" class="btn btn-primary float-right btn-rounded"><i class="fa fa-plus"></i> Add Category</a>
              ...
              <tbody>
                ...
                <?php $count = 1; 
                  if(!empty($categorylist)){
                    foreach ($categorylist as $getdata){ 
                  ?>
                    <tr>
                      <td><?php echo $count; ?></td>
                      <td><?php  echo (!empty($getdata['specialztion'])?$getdata['specialztion']:'none'); ?></td>
...
$(document).ready(function(){
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate category ?");
        if (val == true){
      window.location.reload();
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'category',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate category ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/welcome/change_status",
        data:{tablename:'category',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
